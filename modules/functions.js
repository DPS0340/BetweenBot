...
ifCommandEqualsExpectedRunFunc(command, '핑', (command, expected) => {
    msg.reply('**' + Math.round(client.ping) + 'ms!**');
    return true;
}),
    ifCommandStartsWithExpectedRunFunc(command, '언어변경', (command, expected) => {
        let newlocale = argsParse(expected, command)[0];
        console.log(locale);
    ...
    ifCommandEqualsExpectedRunFunc(command, '데이터 리스트', (command, expected) => {
        if (admin.check(msg.author.id)) {
